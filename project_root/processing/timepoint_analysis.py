...
def aggregate_signals(sessions, timepoint_name, regions_to_aggregate, 
    ...
    if normalize_baseline:
        aggregated_signals -= np.mean(aggregated_signals[:, start_event_idx-7:start_event_idx+7], axis=1, keepdims=True)
    ...
    if aggregated_signals.size > 0:
        ...
        return time_axis, smoothed_mean_signal, lower_bound, upper_bound, (start_event_idx, end_event_idx)
    else:
