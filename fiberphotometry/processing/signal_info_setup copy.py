...
def get_brain_region_event_signal_info(session, event_type, brain_region):
    ...
    # which raw column holds the “true” event time?
    raw_time_col = SYNC['raw_time_col']        # e.g. "Evnt_Time"
    zero_col     = SYNC['sec_zero_col']        # e.g. "SecFromZero"
    phot_times   = phot_df[zero_col].values
    ...
    for i, raw_idx in enumerate(event_type_idxs):
        ...
        event_time = raw_df[raw_time_col].iloc[raw_idx]
