...
from fiberphotometry.utils import count_session_events
import numpy as np
...
def calculate_rate(event_counts, numerator_event, denominator_event, pseudocount=0.5):
    # Add a pseudocount to both numerator and denominator to avoid extreme values.
    numerator = event_counts[numerator_event] + pseudocount
    denominator = event_counts[numerator_event] + event_counts[denominator_event] + 2 * pseudocount
    rate = numerator / denominator if denominator else 0.5
    # print(f"[calculate_rate] {numerator_event}: {event_counts[numerator_event]} + {pseudocount}, "
        #   f"{denominator_event}: {event_counts[denominator_event]} + {pseudocount}, rate: {rate}")
    return rate

def get_hr_and_far(event_counts):
    hit_rate = calculate_rate(event_counts, 'hit', 'miss')
    false_alarm_rate = calculate_rate(event_counts, 'mistake', 'cor_reject')
    # print(f"[get_hr_and_far] hit_rate: {hit_rate}, false_alarm_rate: {false_alarm_rate}")
    
    # Check for extreme rates (0 or 1) that would lead to infinite z-scores.
    if hit_rate in (0, 1) or false_alarm_rate in (0, 1):
        # print("[get_hr_and_far] One of the rates is 0 or 1, returning None, None")
        return None, None

    # Calculate z-scores using the norm.ppf function.
    z_hit = norm.ppf(hit_rate)
    z_false_alarm = norm.ppf(false_alarm_rate)
    # print(f"[get_hr_and_far] z_hit: {z_hit}, z_false_alarm: {z_false_alarm}")
    return z_hit, z_false_alarm

def d_prime(event_counts):
    z_hit, z_false_alarm = get_hr_and_far(event_counts)
    # print(f"[d_prime] event_counts: {event_counts}, z_hit: {z_hit}, z_false_alarm: {z_false_alarm}")
    if z_hit is not None and z_false_alarm is not None:
        result = z_hit - z_false_alarm
        # print(f"[d_prime] result: {result}")
        return result
    else:
        # print("[d_prime] Returning None")
        return None

def c_score(event_counts):
    z_hit, z_false_alarm = get_hr_and_far(event_counts)
    # print(f"[c_score] event_counts: {event_counts}, z_hit: {z_hit}, z_false_alarm: {z_false_alarm}")
    if z_hit is not None and z_false_alarm is not None:
        result = - (z_hit + z_false_alarm) / 2
        # print(f"[c_score] result: {result}")
        return result
    else:
        # print("[c_score] Returning None")
        return None
